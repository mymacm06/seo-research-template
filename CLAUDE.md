...
- Docs drift fix: `website/CLAUDE.MD`'s Brand colour table still listed the pre-2026-04-07 hex values even though `tailwind.config.ts` was repointed to the Midnight & Blue palette months ago. Corrected — no SEO impact, but flagged here since it's the same "docs vs live state" class of issue as the canonical/FAQ work above.
- A batch of SEO-relevant fixes landed in the website repo, several of which directly hit open items in `outputs/seo_checklist.md`:
  - **Doubled title tags fixed** on 19 pages (15 location pages + areas-we-cover, intensive-driving-lessons, automatic-driving-lessons, pass-plus) — titles were rendering e.g. "...| iDrive Auto | iDrive Auto" because page-level titles duplicated the root layout's title template suffix. Directly relevant to the checklist's "unique meta title" quick wins.
  - **Three thin service pages expanded** from ~80 words to 500–700 words each (automatic-driving-lessons, intensive-driving-lessons, pass-plus) — pricing, FAQs with `FAQPage` schema, internal links to location pages. Hits several Content-theme checklist items around topical depth, FAQ schema, and internal linking that were still unchecked.
  - **`Service`/`Offer` JSON-LD schema added** to the same three service pages, mirroring visible pricing (`priceRange` + `Offer` entries) — relevant to the checklist's structured-data and AI-citability guidance (pricing now machine-readable, not just on-page text).
  - **`public/llms.txt` added** — a structured AI-crawler summary (business type, coverage areas, services, key page links) per the llms.txt spec. Directly addresses the checklist's repeated theme of AI/LLM visibility (ChatGPT/Perplexity citation readiness).
  - **Default `og:image` added** (1200×630, branded) — previously no social preview existed on any of the 25 pages, hurting share CTR on WhatsApp/Facebook/LinkedIn/iMessage. Not an explicit checklist line item, but a standard technical SEO/social gap.
  - **`areas-we-cover` meta description shortened** from 212 → 154 characters (was being truncated in search results) — another direct hit on the "unique meta description" quick win.
  - **Basic security headers added** (`X-Content-Type-Options`, `X-Frame-Options`, CSP) — not an SEO item, but Core Web Vitals / trust-signal adjacent; noted for completeness.
- `outputs/seo_checklist.md` was not edited to tick these off — it's a generated tracking artifact from the original transcript synthesis, and several items only partially match (e.g. the meta-description item was fixed on one page, not audited site-wide). Worth a manual pass to update checkboxes if the checklist is still being used to track progress.
- Full file-level detail for all of the above: `website/CLAUDE.MD`, session "2026-07-16 (session 3)".
